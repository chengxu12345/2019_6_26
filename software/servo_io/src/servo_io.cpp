...
int main()
{
	...
	mst.setControlStrategy([&]()
	{
		...
		if (++count % 1000 == 0)
		{
			...
			mst.ecSlavePool().at(0).writePdo(0x7001, 0x02, &value_dq, 8);	//writePdo是写函数，第一个形参是index，第二个形参是subindex，第三个形参写DO的数值，第四个形参表示写操作的bit数
			mst.ecSlavePool().at(0).readPdo(0x6001, 0x01, &value_di, 8);	//readPdo是读函数，第一个形参是index，第二个形参是subindex，第三个形参读DI的数值，第四个形参表示读操作的bit数
		}
	});
